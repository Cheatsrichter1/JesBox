...
- **TV**: an EN/DE chip pair in the top-right of the lobby screen (`Assets/Scripts/Game/Localization.cs`, class `L`). Changing it rebuilds the lobby panel in place and re-picks which language variant of every content bank (trivia questions, solo/vote/draw-prompt text) gets broadcast from then on. All 24 trivia questions, all solo-game titles and instructions, all 8 vote prompts, and all 14 draw prompts have full English and German text — see the `*En`/`*De` field pairs and `Question`/`Title`/`Scenario`/`Answer`/etc. computed properties on each content class.
...
Assets/Scripts/Game/    Unity GameManager (mode state machines) + trivia/vote/solo content banks
Assets/Scripts/Legacy/  Cut features kept for reference (currently: Microgames mode)
...
- **Skip** — force-ends whichever round is currently in progress (question, vote, Chosen One turn, Sketch & Guess draw/guess phase, Bible Charades turn/guess phase) right now, same as if its timer had run out.
...
**Microgames** (the WarioWare-style rapid-fire mode everyone played at once on their own phone) was cut from the active game. It's kept for reference rather than deleted outright:

- `Assets/Scripts/Legacy/Microgames.cs` — the TV-side content bank (`MicrogameDef`/`MicrogameKind`, 6 entries: Manna Rain, Fishers of Men, Joyful Noise, Walk on Water, Loaves and Fishes, Parting the Waters).
- `Assets/Scripts/Legacy/LegacyMicrogameMessages.cs` — the `MicrogamePayload`/`MicrogameRevealPayload` shapes it used to broadcast.
- `phone-ui/src/legacy/MicrogameScreen.jsx` — the phone-side controller for all 6 games (see the comment at the top of the file for exactly what else would need restoring — the `microgame.*` i18n keys and the `fishers-*`/`water-*`/`loaves-*`/`parting-*`/`flash-good`/`flash-bad` CSS classes were removed along with it).

GameManager.cs's mode enum, coroutines (`RunMicrogames`/`RunMicrogameRound`), UI panel, and the `tap`/`submit_score` phone actions were removed outright rather than relocated, since they were tightly coupled to GameManager's internal state — restoring the mode means re-adding that wiring by hand (git history has the exact code if needed), not just dropping the Legacy files back in place.
